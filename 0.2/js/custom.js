...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const latestVersionPath = '/latest/';
    const currentHash = window.location.hash; // Captura cualquier hash presente (#pokemon-interactions, etc.)

    if (!basePath.startsWith(latestVersionPath)) {
      window.location.replace(latestVersionPath + currentHash);
    }
});